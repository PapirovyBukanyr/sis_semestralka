...
/* Weights and biases:
   ...
   - hidden layers: w_hidden[layer][H_SZ][H_SZ] for transitions between hidden layers
...
void neuron_train_step_seq(const double in_seq[INPUT_N * SEQ_LEN], const double target[INPUT_N]){
    double hidden[H_LAYERS][H_SZ];
    neuron_predict_seq(in_seq, (double*)hidden); /* this will compute hidden via predict; we need actual hidden states */
    /* The call above clobbers memory intentionally; compute properly here instead to obtain hidden states. */
    /* recompute hidden while preserving states */
    for(int i=0;i<H_SZ;i++){
        double s = b_hidden[0][i];
        for(int j=0;j<INPUT_N*SEQ_LEN;j++) s += w_input[i][j] * in_seq[j];
        hidden[0][i] = sigmoid(s);
...
int neuron_save_weights(void){
    ...
    /* save input weights */
    for(int i=0;i<H_SZ;i++) for(int j=0;j<INPUT_N*SEQ_LEN;j++) if(fwrite(&w_input[i][j], sizeof(double), 1, f) != 1){ fclose(f); return -1; }
    /* save hidden biases */
    for(int l=0;l<H_LAYERS;l++) for(int i=0;i<H_SZ;i++) if(fwrite(&b_hidden[l][i], sizeof(double), 1, f) != 1){ fclose(f); return -1; }
    /* hidden weights */
    for(int l=0;l<H_LAYERS-1;l++) for(int i=0;i<H_SZ;i++) for(int j=0;j<H_SZ;j++) if(fwrite(&w_hidden[l][i][j], sizeof(double), 1, f) != 1){ fclose(f); return -1; }
    /* output weights and biases */
    for(int k=0;k<INPUT_N;k++) for(int i=0;i<H_SZ;i++) if(fwrite(&w_out[k][i], sizeof(double), 1, f) != 1){ fclose(f); return -1; }
    for(int k=0;k<INPUT_N;k++) if(fwrite(&b_out[k], sizeof(double), 1, f) != 1){ fclose(f); return -1; }
    fclose(f);
    return 0;
}
...
int neuron_load_weights(void){
    ...
    for(int i=0;i<H_SZ;i++) for(int j=0;j<INPUT_N*SEQ_LEN;j++) if(fread(&w_input[i][j], sizeof(double), 1, f) != 1){ fclose(f); return -1; }
    for(int l=0;l<H_LAYERS;l++) for(int i=0;i<H_SZ;i++) if(fread(&b_hidden[l][i], sizeof(double), 1, f) != 1){ fclose(f); return -1; }
    for(int l=0;l<H_LAYERS-1;l++) for(int i=0;i<H_SZ;i++) for(int j=0;j<H_SZ;j++) if(fread(&w_hidden[l][i][j], sizeof(double), 1, f) != 1){ fclose(f); return -1; }
    for(int k=0;k<INPUT_N;k++) for(int i=0;i<H_SZ;i++) if(fread(&w_out[k][i], sizeof(double), 1, f) != 1){ fclose(f); return -1; }
    for(int k=0;k<INPUT_N;k++) if(fread(&b_out[k], sizeof(double), 1, f) != 1){ fclose(f); return -1; }
    fclose(f);
    return 0;
}
